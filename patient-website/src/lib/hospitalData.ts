...
// Helper functions
export function getPatient(id: string) { return MOCK_DB.patients.find(p => p.id === id); }
export function getCasesByTier(tier: string) { return MOCK_DB.cases.filter(c => c.tier === tier && c.status === 'pending'); }
export function getAllPendingCases() { return MOCK_DB.cases.filter(c => c.status === 'pending'); }
export function getCVScreening(id: string) { return (MOCK_DB.cvScreenings as any)[id]; }
export function getVitals(patientId: string) { return (MOCK_DB.vitals as any)[patientId]; }
export function getPrescriptions(patientId: string) { return (MOCK_DB.prescriptions as any)[patientId] || []; }
export function getHistory(patientId: string) { return (MOCK_DB.history as any)[patientId] || []; }
...
export function getTierLabel(tier: string) {
  const labels: Record<string, string> = { red: '🔴 RED', orange: '🟠 ORANGE', yellow: '🟡 YELLOW', green: '🟢 GREEN' };
  return labels[tier] || tier;
}

export function getTierColor(tier: string) {
  return ({ red: 'var(--tier-red)', orange: 'var(--tier-orange)', yellow: 'var(--tier-yellow)', green: 'var(--tier-green)' } as Record<string, string>)[tier] || '#fff';
}

export function getVitalsStatus(vitals: any) {
